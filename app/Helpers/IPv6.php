...
class Net_IPv6
{
        ...
        if (null == $bits) {
            ...
            if (2 == count($elements)) {
                ...
                if (2 == count($elements)) {
                    ...
                    if (
                        preg_match('/^(\:\:[a-f\d]{1,4}|[a-f\d]{1,4}.*\:\:.*[a-f\d]{1,4})$/', $netmask) &&
    ...
    public static function uncompress($ip, $leadingZeros = false)
    {
        ...
        $prefix = Net_IPv6::getNetmaskSpec($ip);
    ...
    public static function checkIPv6($ip)
    {
        ...
        if (!empty($ipPart[0])) {
            ...
            for ($i = 0; $i < count($ipv6); $i++) {
                ...
                $hexStr = preg_replace("/[^0-9A-Fa-f]/", '', $ipv6[$i]);
                $dec = hexdec($hexStr);
    ...
    protected static function _ip2Bin($ip)
    {
        ...
        foreach ($parts as $v) {
            $hexStr = preg_replace("/[^0-9A-Fa-f]/", '', $v);
            $str     = base_convert($hexStr, 16, 2);
